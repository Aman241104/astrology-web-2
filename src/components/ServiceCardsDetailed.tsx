...
export default function ServiceCardsDetailed() {
  return (
    <section className="py-20 md:py-32 bg-[#f4faf7] px-4 relative overflow-hidden">
      {/* Decorative patterns */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20 mb-24 md:mb-32">
          <div className="flex-1 text-center lg:text-left">
            <div className="inline-block bg-white/80 backdrop-blur-sm border border-emerald-100 px-6 py-2 rounded-full mb-6 shadow-sm">
               <span className="text-emerald-800 text-[10px] md:text-xs font-black uppercase tracking-[0.3em]">Spiritual Authority</span>
            </div>
            <h2 className="text-4xl md:text-7xl font-serif font-black text-emerald-950 uppercase tracking-tight mb-8 leading-none">
              Our <span className="text-gradient-gold italic">Top Solutions</span>
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto lg:mx-0 text-lg md:text-xl font-medium leading-relaxed italic mb-8">
              "Every problem has a spiritual remedy." Mohammed Arif Khan provides 100% Halal and effective solutions for all your relationship worries.
            </p>
            <div className="flex flex-wrap justify-center lg:justify-start gap-4">
               ...
               <div className="flex items-center gap-2 bg-white text-emerald-900 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border border-emerald-100 shadow-sm">
                  ...
                  <span>Verified Rohani Expert</span>
               </div>
            </div>
          </div>
          ...
          <div className="flex-1 relative group max-w-md w-full">
            ...
            <div className="w-full aspect-[4/5] rounded-[3rem] overflow-hidden border-[10px] border-white shadow-2xl relative">
              ...
              {/* Overlay Badge */}
              <div className="absolute bottom-6 left-6 right-6 bg-white/95 backdrop-blur-md px-5 py-4 rounded-2xl shadow-xl flex items-center gap-4 border border-emerald-50">
                 <div className="w-12 h-12 bg-gold rounded-full flex items-center justify-center text-emerald-950 shrink-0">
                    <Zap size={24} fill="currentColor" />
                 </div>
                 <div>
                    <p className="text-[10px] font-black text-emerald-900 uppercase tracking-widest leading-none mb-1">Guaranteed Relief</p>
                    <p className="text-xs font-bold text-gray-500">Fast Results in 24 Hours</p>
                 </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-10">
          {cards.map((card, idx) => (
            <div 
              key={idx} 
              className="bg-white rounded-[3rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.05)] flex flex-col transition-all duration-700 hover:shadow-[0_40px_100px_rgba(0,68,0,0.15)] hover:-translate-y-3 group border border-emerald-50/50"
            >
              {/* Image Section */}
              <div className="relative w-full h-56 overflow-hidden">
                ...
                <div className={`absolute -bottom-6 left-1/2 -translate-x-1/2 w-16 h-16 ${card.color} rounded-2xl flex items-center justify-center shadow-2xl transition-all duration-500 group-hover:rotate-[15deg] group-hover:scale-110 border-4 border-white`}>
                  <card.icon size={28} />
                </div>
              </div>
              
              <div className="p-8 md:p-10 pt-12 flex flex-col flex-grow bg-white">
                <h3 className="font-serif text-xl md:text-2xl font-black text-emerald-950 uppercase tracking-tighter leading-tight mb-4 min-h-[3rem] flex items-center justify-center text-center group-hover:text-emerald-700 transition-colors">
                  {card.title}
                </h3>
                
                <p className="text-gray-500 text-sm md:text-base leading-relaxed font-medium mb-6 flex-grow text-center">
                  {card.desc}
                </p>
                ...
                <a 
                  ...
                  className="mt-auto bg-[#25D366] hover:bg-emerald-950 text-white px-8 py-4 rounded-full font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all active:scale-95 shadow-[0_15px_35px_rgba(37,211,102,0.3)] hover:shadow-emerald-900/30 group/btn"
                >
                  <div className="bg-white rounded-full p-1 group-hover/btn:bg-green-500 transition-colors">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-[#25D366] group-hover/btn:fill-white transition-colors" xmlns="http://www.w3.org/2000/svg">
